 ...
 *                   ------- 명령문의 집합: 메소드
 *                   ------- 기능별 처리
 *   데이터 한 개 저장: 변수(지역변수, 전역변수 => 공유 변수, 객체 변수)
 *                     -----  ------ 클래스
 ...
 *      = 다른 클래스와 연결(메세지)
 *        객체지향 프로그램은 여러 개의 클래스를 만들어서 연관 관계
 *        => Scanner...(사용자 정의)
 *           scan.nextInt()
 *                --------- 입력된 정수 읽기
 ...
 *           => 메소드는 한 가지 기능만 수행 가능
 *              1) 입력
 *              2) 요청 처리 => 기능에 따라 세분화(여러 개 메소드 제작)
 *              3) 출력
 *      = 반복 수행을 제거
 *        오라클 연결
 *        --------
 ...
 *        *** 약속 사항 ***
 *        => 소문자로 시작
 *        => 두 개의 단어가 있는 경우 두 번째 단어 첫 자는 대문자 or _
 *        => 데이터 저장: set변수명()...
 *        => 데이터 읽기: get변수명()...
 *   3) 메소드 제작 시 고민
 *      ------------------------------------------
 ...
 *             X                O
 *             => 메소드 자체에서 처리 => 출력
 *             => void main(Stirng[] args) => 리턴형이 없는 경우 => void
 ...
 *          }
 *          ----------------------------------------
 *            메소드 선언 위치(메소드 안에 메소드 선언 불가)
 *          -----------------------------------------
 *          자바는 메소드 클래스 종속 = 메소드
 *          클래스 밖에서 선언 => 함수(C/C++) => 독립적 사용 불가
 *      }
 *      
 *      형식)
 *          리턴형 메소드명(매개변수...)
...
public class 메소드_1 {
	static void a()
	{
		...
		return; // 생략 가능 => 값을 보내줄 때 한 개만 사용 가능
	}
	// 시작점은 main
	public static void main(String[] args) {
