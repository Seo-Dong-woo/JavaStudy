 ...
 *     ---------- 맨 마지막에만 사용하는 것이 아니라 중간에서도 사용 가능
 *     메소드()
 *     {
 *         if()
 *         {
 ...
 *      final: 수정 불가 => String, Math
 *      
 *      리턴형으로 사용 => 기본형(int, byte, long, char, double, boolean)
 *                 => 참조형: 배열/클래스
 *                    String, int[]
 *                 => 메소드 매개변수 전송법
 *                    ----------------
 ...
 *                               ----------
 *                                   | 모든 메소드는 return을 컴파일러가 자동으로 추가
 *      }
 *      
 *      => return/import java.lang.*: 자동 추가
 *                       ----------- String, Math, System
 *         return을 생략할 때는 void인 경우에만 가능
 *      *** 모든 프로그램 ***
 *      = 입력 => 변수에 대한 초기화
 *      = 처리부분(요청) => CRUD
 *                      ----
 ...
 */
// 5개의 정수를 전송 => 정렬 int[]
// 5자리 정수를 전송 => 거꾸로 출력 ex) 12345 => 54321
import java.util.*;
