 ...
 *                => 여러 개를 따로 저장해서 사용 가능
 *                   -------------- new => 중심
 *              공유 변수, 클래스 변수: static => 컴파일러에 의해 자동으로 메모리 생성 => 한 개만 생성
 *              
 *              ex)
 ...
 *                    class A
 *                    {
 *                        A(int a){} => 생성자가 존재 => 자동 추가가 안됨
 *                    }
 *                    
 *                    *** 컴파일러가 자동 생성
 *                    1) import: import java.lnag.*
 *                               ------------------
 *                                       | String, System, Math
 *                    2) void method()
 *                       {
 ...
 *                      => 생성자 호출 시 new 생성자()
 *                    = 모든 클래스는 생성자가 한 개 이상 존재(없는 경우에는 자동 추가)
 *                2) 특징
 *                    = 클래스명과 동일
 *                    = 리턴형이 없음(*** void는 리턴형임 ***)
 ...
 *                      => 기본 public
 *                    = 필요 시에 여러 개의 생성자를 제작할 수 있음
 *                      => 같은 이름으로 여러 개를 만들어서 다른 기능 수행
 *                         -----------------------
 *                                    | 오버로딩(중복함수정의)
 *                                      ------
 *                                       1) 조건
 ...
 *                                ----------- 파일 읽기, 데이터베이스 데이터, 크롤링
 *                   = 시작과 동시에 처리하는 명령문
 *                     ex) 자동 로그인, ID등록, 윈도우 화면 디자인
 ...
 *                }
 *                
 *                static 블록 => 자동으로 메모리 할다 => 생성자를 사용하지 않아도 됨
 *                class A
 *                {
 *                    static int a; // 생성자 호출이 없어도 사용 가능
 *                    static
 *                    {
 *                        a=100;
 *                    }
 ...
 *                  
 *                  static 메소드
 *                  public static 리턴형 메소드명(매개변수) => 선언부
 *                  {
 *                      => 구현부
 *                      => static 메소드나 static변수만 사용 가능
 *                      => 인스턴스 사용 불가
 *                         만약에 사용시에는 반드시 객체 생성 후에 사용 가능
 *                  }
 *                  => 호출 => 클래스명.메소드()
 *                     Math.random()
 *                     
 ...
 *                       => 매개변수의 개수는 제한이 없음 => 가급적 작게 사용
 *                                                  ------------
 *                                                  3개 초과하면 모아서 전송(배열/클래스)
 ...
 *         }
 *         메소드 => 생성자, 초기화블록(구현)
 *         *** 사용자 정의로 만듬 메소드, 생성자는 반드시 호출 해야함 ***
 *         ==> CRUD
 *             회원가입/회원정보/회원 수정/회원 탈퇴
 *             => 면접: 게시판
 *      2) 메모리 저장
 *      3) 활용(호출)
 *   3. 클래스의 종류
...
public class MainClass {
	...
	public static void aaa()
	{
		...
		System.out.println(m.a); // 인스턴스는 객체 생성 후에 사용 가능
