 ...
 *     => 클래스 구조 
 *     1) 형식
 *        class => 필요한 모든 데이터를 모아서 한 번에 저장 목적
 *                          ----------
 *                           기능/데이터
 *        *** 클래스명 => 식별자(키워드 X) ***
 *                  => 약속 => 대문자로 시작
 ...
 *                      사용시) 객체명.변수명
 *                            클래스명.변수명
 *                      저장위치) Method Area(static) => 한 개만 저장 가능
 *                      사용범위) 해당 클래스 전체
 *                             필요시에는 다른 클래스에서도 사용 가능
 *                      소멸시기) 프로그램 종료시까지 사용
 *                ==> 따로 저장/한 개 저장
 ...
 *                                           | 모든 사람이 공유(오라클)
 *            ------------------------
 *              생성자: 반드시 필요(생성자가 없는 경우 자동으로 컴파일러가 생성)
 *                    = 멤버변수 초기화
 *                    = 메모리 할당(클래스 저장) => 호출
 *                    = 시작과 동시에 처리하는 명령문
 *                      (오라클 연결, 네트워크 => 서버 구동, 자동 르그인)
 *                    = 특징
 *                      => 클래스명과 동일
 *                      => 리턴형이 없음
 *                      => 여러 개 만들 수 있음(오버로딩) => 중복 함수 정의
 *                         *** 오버로딩 조건 ***
 *                             1) 한 개의 클래스에서 제작
 *                             2) 메소드명이 동일
 *                             3) 매개변수의 개수나 데이터형이 다름 => 다른 메소드로 인식
 *                             4) 리턴형은 관계없음
 ...
 *                                   ------------ 클래스(3개 이상)
 *                                   ------------------
 *                     => 멤버메소드는 멤버변수, static 변수, 지역변수를 사용 할 수 있음
 *                     => 메소드 유형
 *                        리턴형     매개변수
 *                           ==> R(SELECT) => DML
 *                         O         O
 ...
 *                        
 *                        }
 *                        => static 메소드 안에서는 static 변수, static 메소드 사용이 자유로움
 *                        => static 메소드 안에서는 인스턴스 변수, 메소드는 반드시 객체 생성 후 사용 가능
 *                  = 상속 방지 => final(종단 메소드)
...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		//new A().display();  A()라는 메모리가 두개 생성. 다른 메모리임
		//new A().aaa();      
