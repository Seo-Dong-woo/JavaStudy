...
/*
 *   객체지향의 3대 특성 => 믄법사항이 아님, 권장사함
 *   --------------
 *   class Window extends Frame => 재사용 => 상속
 *     => 변경 사항이 있는 경우
 *   class Window
 *   {
 *       JFrame f=new JFrame() => 재사용 => 포함 => 변경사항이 없음
 *   }
 *   => 웹/스프링 => 상속이 거의 없음 => 포함
 *   
 *   1. 캡슐화: 데이터 보호
 ...
 *         = 중복 제거
 *         = 통일성이 있는 코드 작성
 *         = 단점: 실행속도가 느림(게임 => 자바(X), C/C++(X), 어셈블러)
 *               ------ 최적화 => exe(웹, 모바일)
 *                             J2EE, j2ME => J2SE
 ...
 *           }
 *           
 *           코드를 공통적으로 관리하기 때문에 여러 코드를 추가, 변경 용이
 *                                   ------------ -------
 *                                     오버로딩      오버라이딩
 *                                     
 *           *** 자바는 오버라이딩 => 변경
 ...
 *         => 단점 => 단일 상속만 가능
 *      3) 상속의 특징
 *         1. 생성자, 초기화 블록은 상속이 안됨
 *            상속내린 클래스의 객체 생성시에 => 상속받은 클래스가 먼저 메모리 할당
 *            
 *            class A
 *            {
 *                int a, b;
 ...
 *                   자신 클래스 => this
 *                   
 *      => 상속을 내리는 클래스: 상속받은 클래스에 접근 불가
 *         상속을 받는 클래스: 자신+상속을 내린 클래스에 접근 가능
 *                       this+super
 *      
 *   3. 다형성 => 필요 시
 *       => 변경 됨 => 오버라이딩
 ...
 *         생성자 => [접근지정어] 클래스명()
 *                  public
 *         지역변수, 매개변수: 접근지정어를 사용할 수 없음
 *                        static 사용할 수 없음
 *                        => final만 사용 가능 => 상수형
 *         메소드
 *           멤버메소드: static, 멤버변수를 사용할 수 있음
