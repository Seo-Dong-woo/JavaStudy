 ...
 *                                          => 데이터베이스 연동
 *                                             ------------
 *      = 코드의 재사용이 높음(기존의 코드를 변경, 추가)
 *      = 코드 관리가 용이(구조적 프로그램 => 메소드로 기능 구분)
 *      = 이미 사용중인 프로그램을 이용 => 신뢰성이 높음)
 *      = 대규모 프로젝트에 주로 사용
 *        -----------
 *            | Model 1 => 홈페이지, 소규모 회사
 *              ------- 통으로 프로그램 구현(개인)
 ...
 *                         private
 *              멤버변수: new를 이용할 때마다 따로 저장되는 공간을 만들어서 사용
 *              정적변수: 한 개만 생성 => 공유
 *              => private 데이터형 변수명;
 *                 => 읽기/쓰기
 *                 => 은닉화 VS 캡슐화
 ...
 *                   ==> 웹: 한글변환 ==> ISO-8859_1(ASCII) ==> UTF-8
 *                 = 종단메소드: 오버라이딩이 안되는 메소드 ==> 사용빈도가 거의 없음
 ...
 *                            설계용
 *                            => 추상클래스 / 인터페이스
 *                            => 여러 개의 클래스를 한 개의 이름으로 묶어서 처리
 *                               ----------------------------------
 *                               데이터 여러 개 => 한 개의 이름으로 제어
 *                                             배열
 *                               여러 개의 클래스 => 인터페이스
 *                                                 |
 *                                            결합성을 낮게 만듬
 ...
 *        상속: 기존의 기능을 사용
 *             extends: 확장
 *             => 예외: static, 생성자, 초기화 블록, private은 접근 불가
 *             => final 클래스는 상속할 수 없음
 *        다형성: 변경/추가
 *              => 오버랑리딩 / 오버로딩
 *        ---------------------------------
 ...
 *         ----------------------- 미완성된 클래스
 *                                 -----------
 *                                      | new를 이용해서 메모리 할당이 안됨
 *                                      | 상속을 내려서 구현된 다음에 사용
 *       = 내부클래스: 스레드, 네트워크, 데이터 분석
 *         = 멤버클래스
 *           class A => 서버 => 접속 시 처리   윈도우
 *           {
 *               데이터 => 접속자 IP
 *               class B => 통신서버 => 접속 안하고 통신   스레드
 *               {
 *                   데이터
 *               }
 *           }
